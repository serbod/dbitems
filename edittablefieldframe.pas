...
begin
  if not Assigned(DbFieldInfo) then Exit;
  ...
  if sFt='I' then cbFieldType.ItemIndex:=0
  else if sFt='S' then cbFieldType.ItemIndex:=1
  else if sFt='N' then cbFieldType.ItemIndex:=2
  else if sFt='T' then cbFieldType.ItemIndex:=3
  else if sFt='B' then cbFieldType.ItemIndex:=4
  else if sFt='L' then
  begin
...
begin
  if not Assigned(DbFieldInfo) then Exit;
  if Trim(edFieldName.Text)='' then Exit;

  // set field type
  sFt:='';
  if cbFieldType.ItemIndex=0 then sFt:='I'
  else if cbFieldType.ItemIndex = 1 then sFt:='S'
  else if cbFieldType.ItemIndex = 2 then sFt:='N'
  else if cbFieldType.ItemIndex = 3 then sFt:='T'
  else if cbFieldType.ItemIndex = 4 then sFt:='B'
  else if cbFieldType.ItemIndex = 5 then
  begin
    sFt:='L';
    if Trim(cbLinkedTable.Text)<>'' then sFt:=sFt+'~'+Trim(cbLinkedTable.Text);
  end;
